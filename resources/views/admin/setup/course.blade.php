  ...
  <div class="flex min-h-screen bg-gray-100 w-full">
    @include('admin.sidebar')
          ...
          <div class="bg-white shadow-md rounded px-8 pt-6 pb-8 mb-6 mx-auto">
            <h2 class="text-2xl font-bold mb-6 text-center">Course</h2>

        <form action="{{ route('admin.setup.storecourse') }}" method="POST" enctype="multipart/form-data" class="max-w-xl mx-auto p-6 bg-white rounded-lg shadow-md">
            @csrf
              <div class="mb-4">
                <label class="block text-gray-700 font-bold mb-2" for="course">Course</label>
                <input class="w-full border border-gray-300 rounded px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-400" type="text" id="course" name="course" required>
              </div>
        ...
        <div class="bg-white shadow-md rounded px-8 pt-6 pb-8 mb-6">
          ...
          <div class="flex flex-col md:flex-row justify-between items-center mb-4">
            <input type="text" placeholder="Enter... Course" class="shadow appearance-none border rounded w-full md:w-1/2 py-2 px-3 mb-2 md:mb-0 text-gray-700 focus:outline-none focus:shadow-outline"/>
          </div>
        ...
        <div id="updateModal" class="fixed inset-0 flex items-center justify-center bg-gray-900 bg-opacity-50 hidden">
          <div class="bg-white rounded-lg shadow-lg w-full max-w-2xl p-6 relative">
            ...
            <h2 class="text-2xl font-bold mb-6">Update Course</h2>
            <form id="updateForm" action="{{ route('updatecourse', ['id' => '__ID__']) }}" method="POST" enctype="multipart/form-data">
              @csrf
              @method('PUT')
              ...
              <input type="hidden" name="id" id="course_id" value="">
                ...
                <div>
                  <label class="block text-gray-700 font-bold mb-2" for="edit_course">Course</label>
                  <input type="text" name="course" id="edit_course" class="w-full border rounded px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-400" required>
                </div>
